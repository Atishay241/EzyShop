...
def editcart(uname):
    ...
    q0 = f"Select quantity from cartwithproducts where prodref_id = {pr_id} and cartref_id = {cartid}"
    prv = read_query(connection,q0)
    prvamnt = prv[0][0]

    if quantity>total + prvamnt:
        print("Given quantity is greater than stock!!!")
        return

    q=f"update cartwithproducts as cwp set cwp.quantity={quantity} where prodref_id={pr_id} and cartref_id = {cartid}"
    # connection=create_db_connection("localhost","root",pw,dbname)
    exe_query(connection,q)
    # updatecartamt(uname)
